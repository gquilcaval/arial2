...
public class GestionClientes implements InterfaceCliente {
	...
	public ArrayList<Clientes> listado() {
		...
		try {
			...
			String sql="select tipoPersona,formaDePago,Tipo_documento,N_documento,nomb_cli,nom_comercial,direc_cli,departamento,distrito,telf_cli,celular,estado,correo_cli from cliente;";
			pst=(PreparedStatement) con.prepareStatement(sql);
			rs=pst.executeQuery();
			
			while (rs.next()) {
				Clientes c=new Clientes();
				c.setTip_persona_cli(rs.getString(1));
